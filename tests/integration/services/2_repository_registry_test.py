...
def test_create_repository_registry(test_external_repository, database, cc) -> None:
    ...
    for test_repository in test_external_repository:
        ...
        repository_registry = repository_registry_service.create(
            RepositoryRegistryCreate(**test_repository)
        )

        assert os.path.exists(
            repository_registry_service.git_repo_repository.get_path_physic_repository(
                repository_registry
            )
        )
    ...
    with pytest.raises(RepositoryRegistryError):
        repository_registry_service.create(
            RepositoryRegistryCreate(**test_external_repository[0])
        )
...
def test_get_commits_repository(database, cc) -> None:
    ...
    target_branch = repository_registry_service.git_repo_repository.get_branches(
        target_repository
    )[0]
...
def test_get_set_credentials(test_external_repository, database, cc) -> None:
    ...
    for target_repository_registry in pytest.repository_registries[:1]:
        ...
        credentials = two_repository_registry_service.get_credentials(
            target_repository_registry.uuid
        )
...
def test_update_local_repository(database, cc) -> None:
    ...
    repository_registry_service.git_repo_repository.delete_repo(
        RepositoryRegistry(uuid=target_repository.uuid)
    )
