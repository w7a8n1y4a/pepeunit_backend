...
def test_get_auth_token_user(test_users, cc, database) -> None:
    ...
    with pytest.raises(ValidationError):
        user_service.get_token(
            UserAuth(
                credentials=pytest.users[-1].login + "invalid",
...
async def test_verification_user(database, cc) -> None:
    ...
    with pytest.raises(ValidationError):
        user_service = get_user_service(
            database, cc, pytest.user_tokens_dict[pytest.users[0].uuid]
        )
    ...
    for inc, user in enumerate(pytest.users, start=1):
        user_service = get_user_service(
            database, cc, pytest.user_tokens_dict[user.uuid]
        )
        ...
        await user_service.verification(
            str(random.randint(1_000_000, 10_000_000)), code
        )
...
def test_block_unblock_user(database, cc) -> None:
    user_service = get_user_service(
        database, cc, pytest.user_tokens_dict[pytest.users[-1].uuid]
    )
