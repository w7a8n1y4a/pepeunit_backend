...
def create(
    data: RepoCreate, repo_service: RepoService = Depends(get_repo_service)
):
...
def get_repos(
    ...
    return ReposResult(
        count=count, repos=[RepoRead(**repo.dict()) for repo in repos]
    )
...
@router.patch(
    "/update_units_firmware/{uuid}", status_code=status.HTTP_204_NO_CONTENT
)
