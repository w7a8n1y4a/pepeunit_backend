...
class UnitNodeRepository(BaseRepository):
    ...
    def list(
        self, filters: UnitNodeFilter, restriction: list[str] = None
    ) -> tuple[int, list[UnitNode]]:
        query = self.db.query(UnitNode)

        if filters.output_uuid:
            query = query.join(
                UnitNodeEdge, UnitNodeEdge.node_input_uuid == UnitNode.uuid
