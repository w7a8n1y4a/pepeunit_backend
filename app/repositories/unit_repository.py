    ...
    def list(
        ...
        if is_include_output_unit_nodes:
            if filters.unit_node_input_uuid:
                ...
                query = (
                    self.db.query(
                        Unit, unit_node_by_input_subquery.label("output_nodes")
                    )
                    .select_from(Unit)
                ...
                if filters.unit_node_uuids:
                    unit_node_subquery = unit_node_subquery.filter(
                        unit_node_alias.uuid.in_(filters.unit_node_uuids)
                    )
        ...
        if filters.uuids:
            query = query.filter(
                Unit.uuid.in_([is_valid_uuid(item) for item in filters.uuids])
            )
        ...
        filters.repos_uuids = (
            ...
            if isinstance(filters.repos_uuids, Query)
            else filters.repos_uuids
        )
        if filters.repos_uuids:
            query = query.filter(
                Unit.repo_uuid.in_(
                    [is_valid_uuid(item) for item in filters.repos_uuids]
                )
            )
        if filters.is_auto_update_from_repo_unit is not None:
            query = query.filter(
                Unit.is_auto_update_from_repo_unit
                == filters.is_auto_update_from_repo_unit
            )
