...
def test_check_visibility_private_allowed(authorization_service, mock_agent, mock_permission_repo):
    mock_agent.type = AgentType.USER
    mock_agent.uuid = uuid.uuid4()
    entity = MagicMock()
...
def test_check_visibility_private_disallowed(authorization_service, mock_agent, mock_permission_repo):
    mock_agent.type = AgentType.USER
    mock_agent.uuid = uuid.uuid4()
    entity = MagicMock()
    entity.visibility_level = VisibilityLevel.PRIVATE
...
def test_get_available_visibility_levels_bot(authorization_service, mock_agent):
    mock_agent.type = AgentType.BOT
    result = authorization_service.get_available_visibility_levels(["PUBLIC", "INTERNAL"])
    assert result == [VisibilityLevel.PUBLIC]


def test_get_available_visibility_levels_user_with_restriction(authorization_service, mock_agent):
    mock_agent.type = AgentType.USER
    result = authorization_service.get_available_visibility_levels(["PUBLIC", "INTERNAL"], ["INTERNAL"])
    assert result == ["PUBLIC", "INTERNAL"]


def test_get_available_visibility_levels_user_without_restriction(authorization_service, mock_agent):
    mock_agent.type = AgentType.USER
    result = authorization_service.get_available_visibility_levels(["PUBLIC", "INTERNAL"])
