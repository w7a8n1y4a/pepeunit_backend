...
from app.schemas.gql.types.grafana import DashboardPanelsResultType, DashboardsResultType, DashboardType
...
def get_dashboards(filters: DashboardFilterInput, info: Info) -> DashboardsResultType:
    ...
    return DashboardsResultType(count=count, dashboards=[DashboardType(**dashboard.dict()) for dashboard in dashboards])
...
def get_dashboard_panels(uuid: uuid_pkg.UUID, info: Info) -> DashboardPanelsResultType:
    ...
    return DashboardPanelsResultType(
        count=panels.count, panels=[grafana_service.mapper_panel_to_type(panel) for panel in panels.panels]
    )
