...
def upgrade() -> None:
    ...
    repos = session.execute(
        sa.text(
            "SELECT uuid, cipher_credentials_private_repository, creator_uuid, repository_registry_uuid FROM repos where cipher_credentials_private_repository is not null"
        )
    ).fetchall()
    ...
    if len(repos) > 0:
        ...
        for (
            ...
            current_credentials = is_valid_json(
                aes_gcm_decode(cipher_credentials_private_repository),
                "Old credentials struct",
            )
            ...
            if (
                str(creator_uuid)
                not in registry_credentials[str(repository_registry_uuid)]
            ):
                registry_credentials[str(repository_registry_uuid)][
                    str(creator_uuid)
                ] = {"credentials": current_credentials, "status": "NotVerified"}

        for repository_registry_uuid, struct in registry_credentials.items():
            new_cipher = aes_gcm_encode(json.dumps(struct))
            session.execute(
                sa.text(
                    f"UPDATE repository_registry SET cipher_credentials_private_repository = '{new_cipher}' WHERE uuid = '{str(repository_registry_uuid)}'"
                )
            )
